import filter_integration as fi
import gaussian_process as gp
import extinction_correction as extcorr
import mangling  # for K-correction
...
def initialise(file_name):
    ...
    sn_obj.bands = [band for band in list(sn_file['band'].unique()) if len(sn_file[sn_file['band']==band]['flux'].values) >= 3]
    sn_obj.call_filters()
...
def init_sn(str, directory='./'):
    """Initialise a supernova object with the data from the given file/name.
    ...
    if os.path.isfile(directory+str):
        ...
        try:
            return initialise(directory+str+'_flux.dat')
        except:
...
class sn(object):
    ...
    def call_filters(self):  
        ...
        vega_wave, vega_flux = np.loadtxt('templates/alpha_lyr_stis_005.ascii').T
            ...
            for root, dirs, files in os.walk('./filters/'):
                if file in files:
                    ...
                    self.filters[band] = {'wave':wave, 
                                          ...
                                          'eff_wave':fi.calc_eff_wave(vega_wave, vega_flux, wave, 
                                                                   transmission, response_type=response_type),
                                          'pivot_wave':fi.calc_pivot_wave(wave, transmission, 
                                                                       response_type=response_type),
                                          'response_type':response_type}
        
        # add Bessell filters        
        file_paths = [file for file in glob.iglob('./filters/Bessell/*.dat')]
        ...
        for file_path in file_paths:
            ...
            self.filters[band] = {'wave':wave, 
                                  'transmission':transmission, 
                                  'eff_wave':fi.calc_eff_wave(vega_wave, vega_flux, wave, transmission, 
                                                           response_type=response_type),
                                  'pivot_wave':fi.calc_pivot_wave(wave, transmission, response_type=response_type),
                                  'response_type':response_type}
    ...
    def add_filters(self, filter_list, response_type='photon'):
        ...
        if isinstance(filter_list, str) and os.path.isdir(f'filters/{filter_list}'):
            # add directory
            path = f'filters/{filter_list}'
    ...
    def print_sed_templates(self):
        ...
        print('The list of available SED templates are:', [name for name in os.listdir("./templates/") 
                                                           if os.path.isdir(f"./templates/{name}")])
    ...
    def set_sed_template(self, template='conley09f'): 
        ...
        file = f'./templates/{template}/snflux_1av2.dat'  # v2 is the interpolated version to 0.5 day steps
    ...
    def set_sed_epoch(self, set_eff_wave=True): 
        ...
        for band in self.bands:
            flux = fi.run_filter(self.sed['wave'], self.sed['flux'], self.filters[band]['wave'], 
    ...
    def set_eff_wave(self):
        ...
        for band in self.filters.keys():
            self.filters[band]['eff_wave'] = fi.calc_eff_wave(self.sed['wave'], self.sed['flux'], 
    ...
    def test_sed_flux(self, verbose=False):
        ...
        for band in self.filters.keys():
            ...
            min_idx, max_idx = fi.filter_effective_range(self.filters[band]['transmission'])
    ...
    def fit_lcs(self, kernel='matern52'):
        ...
        for band in self.bands:
            time, flux, std = gp.fit_gp(self.data[band]['mjd'], self.data[band]['flux'], 
        ...
        B_min, B_max = fi.filter_effective_range(self.filters['Bessell_B']['transmission'])
        ...
        range_indexes = {band:fi.filter_effective_range(self.filters[band]['transmission']) for band in valid_bands}
    ...
    def mask_data(self, band_list=None, mask_snr=True, snr=3, mask_phase=False, min_phase=-20, max_phase=40):
        ...
        if mask_snr:        
            for band in band_list:
                mask = np.where(self.data[band]['flux']/self.data[band]['flux_err'] >= snr)
        ...
        if mask_phase:   
            ...
            for band in band_list:
                mask = np.where(np.logical_and(self.data[band]['mjd'] - self.tmax >= min_phase*(1+self.z), 
    ...
    def integrate_filters(self, band_list=None, value_type='flux'):  
        ...
        idxs = {band:fi.filter_effective_range(self.filters[band]['transmission']) for band in band_list}
        
        if value_type=='flux':
            return {band:fi.run_filter(self.sed['wave'], self.sed['flux'], self.filters[band]['wave'], 
                                    self.filters[band]['transmission'], self.filters[band]['response_type']) 
        ...
        mags = {band:-2.5*np.log10(fi.run_filter(self.sed['wave'], self.sed['flux'], self.filters[band]['wave'], 
        ...
        mags.update({band:-2.5*np.log10(fi.run_filter(self.sed['wave'], self.sed['flux'], self.filters[band]['wave'], 
    ...
    def normalize_data(self):
        ...
        for band in self.bands:
            ...
            if mag_sys=='VEGA':
                zp = fi.calc_zp_vega(self.filters[band]['wave'], 
                                  self.filters[band]['transmission'], 
                                  self.filters[band]['response_type'])
            elif mag_sys=='AB':
                zp = fi.calc_zp_ab(self.filters[band]['pivot_wave'])
    ...
    def mangle_sed(self, kernel='squaredexp', method='gp'):
        ...
        results = mangling.mangle(flux_ratios, flux_ratios_err, self.sed['wave'], self.sed['flux'], 
        ...
        for band, obs_flux in zip(valid_bands, obs_fluxes):
            ...
            model_flux = fi.run_filter(mangled_wave, mangled_flux, band_wave, band_transmission, response_type)
    ...
    def correct_extinction(self, scaling=0.86):
        ...
        self.sed['flux'] = extcorr.deredden(self.sed['wave'], self.sed['flux'], self.ra, self.dec, scaling=scaling)
        self.sed['flux_err'] = extcorr.deredden(self.sed['wave'], self.sed['flux_err'], self.ra, self.dec, scaling=scaling)
        ...
        for band in self.filters.keys():
            ...
            if ( self.sed['wave'].max() >= self.filters[band]['wave'][max_idx] ) and ( self.sed['wave'].min() <= self.filters[band]['wave'][min_idx] ):
                ...
                if band in self.bands:
                    ...
                    zp = fi.calc_zp_vega(self.filters[band]['wave'], self.filters[band]['transmission'], self.filters[band]['response_type'])
                    mag_sys = 'VEGA'    
                                   
                band_flux = fi.run_filter(self.sed['wave'], self.sed['flux'], 
                                       self.filters[band]['wave'], self.filters[band]['transmission'], self.filters[band]['response_type'])
                band_flux_err = fi.run_filter(self.sed['wave'], self.sed['flux_err'], 
                                           self.filters[band]['wave'], self.filters[band]['transmission'], self.filters[band]['response_type'])
    ...
    def check_peak(self, threshold=0.2, iter_num=1, maxiter=5, scaling=0.86, **mangle_kwargs):
        ...
        **mangle_kwargs : 
        ...
        B_band = 'Bessell_B'
        phase, flux, _ = gp.fit_gp(self.lc_correct[B_band]['phase'], self.lc_correct[B_band]['flux'], self.lc_correct[B_band]['flux_err'])
        ...
        if (iter_num <= maxiter) and (np.abs(phase_max) > threshold):
            ...
            self.check_peak(threshold=threshold, iter_num=iter_num+1, maxiter=maxiter, scaling=scaling, **mangle_kwargs)
    ...
    def calc_lc_parameters(self):
        ...
        try:
            ...
            #phaseV, fluxV, flux_errV = gp.fit_gp(self.lc_correct[V_band]['phase'], self.lc_correct[V_band]['flux'], self.lc_correct[V_band]['flux_err'])
        ...
        for band in self.filters.keys():   
            try: # is this "try" even necessary?
                fit_gp_results = gp.fit_gp(self.lc_correct[band]['phase'], 

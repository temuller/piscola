...
def residual(params, eff_waves, flux_ratio_err, sed_wave, sed_flux, obs_flux, obs_flux_err, bands, filters, kernel, method):
    ...
    if method=='gp':
        x_pred, y_pred, yerr_pred = gp.fit_gp(eff_waves, flux_ratio, flux_ratio_err, kernel=kernel, 
                                           mangling=True, x_edges=[min_wave, max_wave])
    elif method=='spline':
        x_pred, y_pred, yerr_pred = spl.fit_spline(eff_waves, flux_ratio, flux_ratio_err, x_edges=[min_wave, max_wave])
    
    interp_sed_flux = np.interp(x_pred, sed_wave, sed_flux)
    mangled_wave, mangled_flux = x_pred, y_pred*interp_sed_flux
    print('hola')
    model_flux = np.array([fi.run_filter(mangled_wave, mangled_flux, filters[band]['wave'], filters[band]['transmission'], 
...
def mangle(flux_ratio, flux_ratio_err, sed_wave, sed_flux, bands, filters, obs_fluxes, obs_flux_err, kernel='squaredexp', method='gp'):
    ...
    if method=='gp':
        x_pred, y_pred, yerr_pred = gp.fit_gp(eff_waves, opt_flux_ratio, flux_ratio_err, kernel=kernel,
                                           mangling=True, x_edges=[min_wave, max_wave])
    elif method=='spline':
        kernel = None  # for result display purposes
        x_pred, y_pred, yerr_pred = spl.fit_spline(eff_waves, opt_flux_ratio, flux_ratio_err, x_edges=[min_wave, max_wave])
    ...
    for band, obs_flux in zip(bands, obs_fluxes):
        model_flux = fi.run_filter(mangled_wave, mangled_flux, 
    ...
    for band in bands:
        band_flux = fi.run_filter(sed_wave, sed_flux, 
                               filters[band]['wave'], filters[band]['transmission'], filters[band]['response_type'])
